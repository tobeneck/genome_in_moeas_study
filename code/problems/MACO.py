...
def f1(x) -> np.float64:
    return np.float64(np.sum(np.abs(x)) / len(x))


def f2(x, p: float = None, weight=None, classes=None) -> np.float64:
    if classes is None:
...
def fitness(x:np.array, p : np.float64 = -np.inf, weight : np.array = None, classes: np.array = None):
    if weight is None:
...
class MultiAgentCoordinationProblem(ElementwiseProblem):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        out["F"] = np.column_stack(fitness(x, p=self.p, weight=self.weights, classes=self.classes)).astype(np.float64)
