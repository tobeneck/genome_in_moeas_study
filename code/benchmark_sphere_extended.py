...
def generate_seed_individuals(initial_pop: np.array, problem: Sphere, n_var: int):
    ...
    seed_inds[18] = generate_sphere_individual(n_var, 0.0, worst)
    seed_inds[19] = generate_sphere_individual(n_var, 0.25, worst)
    seed_inds[20] = generate_sphere_individual(n_var, 0.5, worst)
    seed_inds[21] = generate_sphere_individual(n_var, 0.75, worst)
    ...
    for crossover_name in crossovers.keys():
        ...
        for seed_ind_index in range(len(seed_individuals)):
            ...
            for i in range(31):#31 re-runs as usual
                ...
                #print output:
                callback.finalize("../data/sphere_function_data")